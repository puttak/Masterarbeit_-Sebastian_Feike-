...
def bidirectional_RNN(params, dropout, x_input, seqlen):
    '''
    ...
    '''
    # represent the different batch length of training, evaluation and predition iterations
    batch_len = tf.shape(x_input)[0]

    with tf.name_scope("deep_stacked_RNN_Layers"):
        forward_cells = []
        backward_cells = []
        fw_init_state = []
        bw_init_state = []
        for layer in range(params["num_layer"]):
            if params["rnn_cell"] == "Vanilla":
                ...
                rnn_layer = tf.contrib.rnn.LSTMCell(params["num_hidden"], activation=params["activation"],
                                                    state_is_tuple=True)
                rnn_layer = tf.contrib.rnn.DropoutWrapper(rnn_layer, dropout)

            elif params["rnn_cell"] == "GRU":
                # create num_layer GRUCells
                rnn_layer = tf.contrib.rnn.GRUCell(params["num_hidden"], activation=params["activation"])
                rnn_layer = tf.contrib.rnn.DropoutWrapper(rnn_layer, dropout)
            ...
            #
            forward_cells.append(rnn_layer)
            backward_cells.append(rnn_layer)
            # initialize the zero_state for each layer
            init_state = rnn_layer.zero_state(batch_len, tf.float32)
            # save the zero_state for the forward and backward process
            fw_init_state.append(init_state)
            bw_init_state.append(init_state)

    rnn_outputs, state_fw, state_bw = tf.contrib.rnn.stack_bidirectional_dynamic_rnn(forward_cells, backward_cells,
                                                                                     x_input,
                                                                       initial_states_fw=fw_init_state,
                                                                       initial_states_bw=bw_init_state,
...
def model(params, train_temp, train_res, test_temp, test_res, hparam):
    ...
    weights = {'out': tf.Variable(tf.truncated_normal(shape=[2 * params["num_hidden"], 1],
                                                      stddev=params["weight_stddev"]), name="output_weights")}
    ...
    # 2) Call the ANNs
    ################################################################################################################
    # call the deep stacked RNN
    rnn_outputs = bidirectional_RNN(params, dropout, x_input, seqlen)
                            ...
                            for learning_rate in [0.001]:
                                ...
                                for activation in [None]:
                                    ...
                                    for dropout_rate in [0.8]:
                                        for delta in [0.001]: # tends stronger regularization for Bidir-RNNs
                                            if rnn_cell == "GRU":
                                                delta = 0.0001
